...
export default function AppHeader({ title, onMenuClick }: AppHeaderProps) {
  ...
  return (
    ...
    >
      <div className="lg:ml-0 px-3 sm:px-6 lg:px-8 pt-[env(safe-area-inset-top)]">
        <div className="flex justify-between items-center h-14 min-h-14 sm:h-[72px]">
          ...
          <div className="flex items-center gap-2 sm:gap-4 min-w-0">
            ...
            {title && (
              <h1 className="font-display text-lg font-bold text-gray-900 sm:hidden truncate">{title}</h1>
            )}
          </div>
          ...
          <div className="flex items-center gap-1 sm:gap-4 flex-shrink-0">
            ...
            <button
              ...
              className="px-4 py-2.5 min-h-[44px] text-sm text-white bg-red-500 hover:bg-red-600 active:bg-red-700 rounded-full font-medium shadow-md hover:shadow-lg active:scale-[0.98] transition-all duration-200"
            >
              {t('common.signOut')}
            </button>
          </div>
