...
const STORAGE_KEY = '__offerCart_snapshot_v1';
...
export function setOfferCart(items: StoredCartItem[]): void {
  if (typeof window === 'undefined') return;
  // In-memory (keeps File objects in the original cart on the page that set it).
  window[KEY] = items;

  // Serializable snapshot for navigation / reload within the same tab.
  try {
...
export function getOfferCart(): StoredCartItem[] | null {
